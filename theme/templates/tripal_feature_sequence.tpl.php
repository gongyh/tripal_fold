 ...
 */
?>
<div>This is a test div element</div>
<?php
...
if ($residues or count($featureloc_sequences) > 0) { 
   ...
   */
  if( ($feature->type_id->name != "gene") && (count($featureloc_sequences) > 0) ){
  ...
  if($result){
    ...
    drupal_add_js(drupal_get_path('module', 'tripal_fold') . '/theme/js/tripalFoldDraw.js');
    drupal_add_js(array('tripal_fold' => array('poc_loc' => $poc_loc)), array('type' => 'setting'));
  }
}
